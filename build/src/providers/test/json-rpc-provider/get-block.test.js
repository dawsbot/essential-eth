...
function testBlockEquality(eeBlock, otherBlock) {
    ...
    if (typeof otherBlock.gasLimit === 'number') {
        ...
        if (eeBlock.transactions && typeof eeBlock.transactions[0] !== 'string') {
            eeBlock.transactions.forEach((transaction)=>{
            ...
            otherBlock.transactions.forEach((transaction)=>{
                if (transaction.gas) transaction.gas = transaction.gas.toString();
    ...
    const omittedEEBlock = (0, _justOmit.default)(eeBlock, omittableEE);
    const omittedOtherBlock = (0, _justOmit.default)(otherBlock, omittableOther);
...
describe('provider.getBlock', ()=>{
    ...
    const blockHash = '0x4cbaa942e48a91108f38e2a250f6dbaff7fffe3027f5ebf76701929eed2b2970'; // Hash corresponds to block on RSK Mainnet
...
describe('provider.getBlock error handling', ()=>{
    it('should handle empty 200 http response', async ()=>{
        expect.assertions(1);
        const essentialEth = new _.JsonRpcProvider(_rpcUrls.fakeUrls.notRPCButRealHttp);
