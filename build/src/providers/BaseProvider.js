...
const _cleanBlock = require("../classes/utils/clean-block");
const _cleanLog = require("../classes/utils/clean-log");
const _cleanTransaction = require("../classes/utils/clean-transaction");
const _cleanTransactionReceipt = require("../classes/utils/clean-transaction-receipt");
const _fetchers = require("../classes/utils/fetchers");
const _hexToDecimal = require("../classes/utils/hex-to-decimal");
const _prepareTransaction = require("../classes/utils/prepare-transaction");
const _logger = require("../logger/logger");
const _tinyBig = require("../shared/tiny-big/tiny-big");
const _chainsInfo = /*#__PURE__*/ _interopRequireDefault(require("./utils/chains-info"));
 ...
 */ function prepBlockTag(blockTag) {
    return typeof blockTag === 'number' ? (0, _tinyBig.tinyBig)(blockTag).toHexString() : blockTag;
}
class BaseProvider {
    /**
   ...
   * ```
   */ async getNetwork() {
        const hexChainId = await this.post((0, _fetchers.buildRPCPostBody)('eth_chainId', []));
        const chainId = (0, _hexToDecimal.hexToDecimal)(hexChainId);
        const info = _chainsInfo.default[chainId];
        return {
   ...
   */ async getBlockNumber() {
        const currentBlockNumber = await this.post((0, _fetchers.buildRPCPostBody)('eth_blockNumber', []));
        return Number((0, _hexToDecimal.hexToDecimal)(currentBlockNumber));
    }
    /**
   ...
   */ async getTransaction(transactionHash) {
        const [rpcTransaction, blockNumber] = await Promise.all([
            this.post((0, _fetchers.buildRPCPostBody)('eth_getTransactionByHash', [
                transactionHash
            ])),
            this.getBlock('latest')
        ]);
        const cleanedTransaction = (0, _cleanTransaction.cleanTransaction)(rpcTransaction);
   ...
   */ async getTransactionReceipt(transactionHash) {
        const [rpcTransaction, blockNumber] = await Promise.all([
            this.post((0, _fetchers.buildRPCPostBody)('eth_getTransactionReceipt', [
                transactionHash
            ])),
            this.getBlock('latest')
        ]);
        const cleanedTransactionReceipt = (0, _cleanTransactionReceipt.cleanTransactionReceipt)(rpcTransaction);
   ...
   */ async getTransactionCount(address, blockTag = 'latest') {
        ...
        const transactionCount = await this.post((0, _fetchers.buildRPCPostBody)('eth_getTransactionCount', [
            address,
            blockTag
        ]));
        return Number((0, _hexToDecimal.hexToDecimal)(transactionCount));
    }
    /**
   ...
   */ async getBlock(timeFrame = 'latest', returnTransactionObjects = false) {
        ...
        const rpcBlock = await this.post((0, _fetchers.buildRPCPostBody)(`eth_getBlockBy${type}`, [
            timeFrame,
            returnTransactionObjects
        ]));
        return (0, _cleanBlock.cleanBlock)(rpcBlock, returnTransactionObjects);
    }
    /**
   ...
   * ```
   */ async getGasPrice() {
        const hexGasPrice = await this.post((0, _fetchers.buildRPCPostBody)('eth_gasPrice', []));
        return (0, _tinyBig.tinyBig)((0, _hexToDecimal.hexToDecimal)(hexGasPrice));
    }
    /**
   ...
   */ async getBalance(address, blockTag = 'latest') {
        blockTag = prepBlockTag(blockTag);
        const hexBalance = await this.post((0, _fetchers.buildRPCPostBody)('eth_getBalance', [
            address,
            blockTag
        ]));
        return (0, _tinyBig.tinyBig)((0, _hexToDecimal.hexToDecimal)(hexBalance));
    }
    /**
   ...
   */ async getCode(address, blockTag = 'latest') {
        blockTag = prepBlockTag(blockTag);
        const contractCode = await this.post((0, _fetchers.buildRPCPostBody)('eth_getCode', [
            address,
            blockTag
        ]));
        return contractCode;
   ...
   */ async estimateGas(transaction) {
        const rpcTransaction = (0, _prepareTransaction.prepareTransaction)(transaction);
        const gasUsed = await this.post((0, _fetchers.buildRPCPostBody)('eth_estimateGas', [
            rpcTransaction
        ]));
        return (0, _tinyBig.tinyBig)((0, _hexToDecimal.hexToDecimal)(gasUsed));
    }
    /**
   ...
   */ async getLogs(filter) {
        ...
        const rpcLogs = await this.post((0, _fetchers.buildRPCPostBody)('eth_getLogs', [
            filter
        ]));
        const logs = rpcLogs.map((log)=>(0, _cleanLog.cleanLog)(log, false));
        return logs;
    }
    /**
   ...
   */ async call(transaction, blockTag = 'latest') {
        if (transaction.gasPrice && (transaction.maxPriorityFeePerGas || transaction.maxFeePerGas)) {
            _logger.logger.throwError('Cannot specify both "gasPrice" and ("maxPriorityFeePerGas" or "maxFeePerGas")', {
        ...
        if (transaction.maxFeePerGas && transaction.maxPriorityFeePerGas) {
            _logger.logger.throwError('Cannot specify both "maxFeePerGas" and "maxPriorityFeePerGas"', {
                maxFeePerGas: transaction.maxFeePerGas,
                maxPriorityFeePerGas: transaction.maxPriorityFeePerGas
            });
        }
        blockTag = prepBlockTag(blockTag);
        const rpcTransaction = (0, _prepareTransaction.prepareTransaction)(transaction);
        const transactionRes = await this.post((0, _fetchers.buildRPCPostBody)('eth_call', [
            rpcTransaction,
            blockTag
        ]));
   ...
   */ this._rpcUrls = [];
        /**
   * @ignore
   */ this._post = (body)=>(0, _fetchers.post)(this.selectRpcUrl(), body);
